...
% Generate sequence 2..N see https://wbear.wordpress.com/2011/12/08/prime-numbers-with-erlang/
generateList(N) -> generateList(N, 2).
generateList(Max, Max) -> [Max];
generateList(Max, X) -> 
	[X | generateList(Max, X + 1)].

%% * Multiples
multiples_as_list(0) -> [0];
multiples_as_list(N) when N > 0 -> 
	multiples_as_list_ext(N, 1).

multiples_as_list_ext(_, X) when X > 100 -> [];
multiples_as_list_ext(N, X) when X =< 100 -> 
	[N * X | multiples_as_list_ext(N, X + 1)].

multiples_in_shell(0) -> [0];
multiples_in_shell(N) when N > 0 -> 
	multiples_in_shell_ext(N, 1).

multiples_in_shell_ext(_, X) when X > 100 -> 0;
multiples_in_shell_ext(N, X) when X =< 100 -> 
	io:fwrite("\nMultiple for ~w : ~w", [X, N * X]), 
	multiples_in_shell_ext(N, X + 1).

%% * RingBuffer
ring_buffer(N, List) -> 
	remove_last_element([N|List]).

% is equal to is last element, if yes remove it
remove_last_element([_T])  -> [];
remove_last_element([H|T]) -> 
	[H|remove_last_element(T)].

% * show last element
show_last_element([_T]) -> 
	io:fwrite("\nLast element is ~w in list ", [_T]), 
	_T;
show_last_element([H|T]) -> [H|show_last_element(T)].
% * get last element
get_last_element([_T]) when 1 == length([_T]) -> 
	io:fwrite("Last element is ~c ", [_T]), 
	_T;
get_last_element(List) -> 
	get_last_element(tl(List)).